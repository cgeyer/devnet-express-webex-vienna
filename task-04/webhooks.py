...
def registerWebhook(url):
  """Registers an event listener for attachments to the specified URL."""

  print('registering new webhook to URL %(url)s' % {'url' : url})

  api = WebexTeamsAPI(access_token=teams_token)
  api.webhooks.create(name='attachment-listener', targetUrl=url, resource='attachmentActions', event='created')
